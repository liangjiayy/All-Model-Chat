            ...
            if (urlStr.includes(TARGET_HOST)) {
                try {
                    ...
                    const isContentEndpoint =
                        currentProxyUrl.includes(':streamGenerateContent') ||
                        currentProxyUrl.includes(':generateContent');

                    if (isContentEndpoint) {
                        // 获取"/model:stream"部分，如/gemini-2.5-flash:generateContent
                        const end = urlStr.substring(urlStr.lastIndexOf('/'));
                        newUrl =
                        currentProxyUrl.slice(
                            0,
                            currentProxyUrl.lastIndexOf('/')
                        ) + end;
                    }
                    else {
